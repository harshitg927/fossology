...
class ChangeLicenseBulk extends DefaultPlugin
{
  ...
  private function importBulkDataToCustomPhrase($bulkId, $userId, $groupId)
  {
    ...
    // Fetch associated licenses from license_set_bulk (only non-removing licenses)
    $licensesSql = "SELECT rf_fk FROM license_set_bulk
                    WHERE lrb_fk = $1 AND (removing = false OR removing IS NULL)";
    ...
    while ($licenseRow = $this->dbManager->fetchArray($licensesResult)) {
      $addedLicenses[] = intval($licenseRow['rf_fk']);
    }
    ...
    try {
      ...
      $params = array($refText, $textMd5, null, null, $userId, $groupId, 'true');
      ...
      if (!empty($addedLicenses)) {
        $mapSql = "INSERT INTO custom_phrase_license_map (cp_pk, rf_pk) VALUES ($1, $2)";
        $this->dbManager->prepare($mapStmt = __METHOD__ . ".insertLicenseMap", $mapSql);

        foreach ($addedLicenses as $licenseId) {
          $mapResult = $this->dbManager->execute($mapStmt, array($cpPk, $licenseId));
